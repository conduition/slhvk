...
const uint64_t TREE_ADDRESS_MASK = ((uint64_t) 1 << (HYPERTREE_HEIGHT - XMSS_HEIGHT)) - 1;
const uint32_t SIGNING_KEYPAIR_ADDRESS_MASK = (1 << XMSS_HEIGHT) - 1;
...
void slhvkMessagePrf(
  ...
  sha256_finalize(&hmacHashState, randomizer, N);
}
...
void slhvkDigestAndSplitMsg(
  ...
  ShaContext shaCtx;
  sha256_init(&shaCtx);
  sha256_update(&shaCtx, randomizer, N);
  sha256_update(&shaCtx, pkSeed, N);
  ...
  uint8_t fullMessageDigest[M];
  uint8_t mgfCounter[4] = {0, 0, 0, 0};

  #if M <= 32
    sha256_update(&shaCtx, mgfCounter, 4);
    sha256_finalize(&shaCtx, fullMessageDigest, M);
  #elif M <= 64
    sha256_clone(&shaCtxInner, &shaCtx);
    sha256_update(&shaCtxInner, mgfCounter, 4);
    sha256_finalize(&shaCtxInner, fullMessageDigest, M);

    mgfCounter[3] += 1;
    sha256_update(&shaCtx, mgfCounter, 4);
    sha256_finalize(&shaCtx, &fullMessageDigest[32], M - 32);
  #else
    #error "Unexpected message hash length, should be M <= 64"
  #endif

  uint8_t forsDigest[FORS_DIGEST_SIZE];
  uint64_t treeAddress = 0;
  uint32_t signingKeypairAddress = 0;

  size_t digestBytesRead = 0;
  for (int i = 0; i < FORS_DIGEST_SIZE; i++) {
  ...
  memset(forsIndices, 0, FORS_TREE_COUNT * sizeof(uint32_t));
  uint32_t bitsProcessed = 0;
  for (uint32_t i = 0; i < FORS_TREE_COUNT; i++) {
    for (uint32_t j = 0; j < FORS_TREE_HEIGHT; j++) {
      ...
      uint32_t bitIndexInBaseAWord = bitsProcessed % FORS_TREE_HEIGHT;
      forsIndices[i] |= t << (FORS_TREE_HEIGHT - 1 - bitIndexInBaseAWord);
      bitsProcessed += 1;
    }
  }
